...
- (void)dealloc {
	...
	if([self fadeOutTimer] != nil)
		[[self fadeOutTimer] invalidate], [self setFadeOutTimer:nil];
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
   ...
   [self setFadeOutTimer:[NSTimer scheduledTimerWithTimeInterval:seconds target:self selector:@selector(dismiss) userInfo:nil repeats:NO]];
}
